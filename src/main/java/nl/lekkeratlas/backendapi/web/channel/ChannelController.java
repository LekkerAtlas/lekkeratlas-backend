            ...
            try (Dao<User, UUID> userDao = DAOFactory.createDAO(connection, User.class)) {
                ...
                try (Dao<QueueJob, UUID> queueJobDao = DAOFactory.createDAO(connection, QueueJob.class)){
                    List<QueueJob> existingJobs = new QueryBuilder<>(queueJobDao)
                            .where(QueueJob.class.getDeclaredField("dedupeKey"), SingleValueOperator.EQUALS, command.dedupeKey())
                            // TODO Add OR when implemented in Fluid JDBC
//                            .and(QueueJob.class.getDeclaredField("status"), SingleValueOperator.EQUALS, QueueJobStatus.QUEUED)
//                            .or(QueueJob.class.getDeclaredField("status"), SingleValueOperator.EQUALS, QueueJobStatus.RUNNING)
                            .get();
